...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedStudents = students.flatMap((student) => {
      ...
      if (student.platform_connections.length === 0) {
        return [{
          ...
          last_active: student.stats_snapshots[0]?.captured_at || null,
          stats: {
            rapidRating: null,
            blitzRating: null,
            puzzleRating: null,
      ...
      return student.platform_connections.map((connection) => {
        ...
        // Debug logging for "robozhok" nickname
        const nickname = student.username || student.full_name || "Unnamed";
        if (nickname.toLowerCase() === 'robozhok') {
          console.log(`[coach/students] [DEBUG] robozhok row: id=${result.id}, platform=${platform}, key=${student.id}:${platform}`);
        }
